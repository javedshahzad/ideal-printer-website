...
class Hide_Admin_Notices {
    ...
    public function show_test_admin_notice_for_all_user_roles() {
        ...
        <div class="notice notice-info is-dismissible">
            <p><?php esc_html_e( "This notice is visible for all user roles.", 'wpturbo' ); ?></p>
        </div>
        <?php        
    }
    ...
    public function admin_notices_menu( WP_Admin_Bar $wp_admin_bar ) {
        ...
        if ( is_admin() && ! is_customize_preview() ) {
            ...
            // If the option to hide the menu for non-admins is enabled, and the current user is not an administrator, return early
            if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
                if ( $hide_menu_for_nonadmins && ! current_user_can( 'manage_options' ) ) {
                    return;
                }
            }
