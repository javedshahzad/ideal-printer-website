...
class Image_Upload_Control {
    ...
    public function image_upload_handler( $upload ) {
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
            if ( ! $this->should_apply_image_upload_control__premium_only( $options ) ) {
                return $upload;
            }   
        }
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
            $disable_image_conversion = isset( $options['disable_image_conversion'] ) ? $options['disable_image_conversion'] : false ;
        }
        ...
        if ( in_array( $upload['type'], $applicable_mime_types ) ) {
            ...
            if ( 
                ...
                if ( ! is_wp_error( $wp_image_editor ) ) {
                    ...
                    if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
                        $convert_to_jpg_quality = isset( $options['convert_to_jpg_quality'] ) ? intval( $options['convert_to_jpg_quality'] ) : 82 ;
                    } else {
    ...
    public function prefilter_maybe_fix_image_orientation( $file ) {
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
            if ( ! $this->should_apply_image_upload_control__premium_only() ) {
                return $file;
            }   
        }
    ...
    /**
     * Disable the generation of auto-generated intermediate sizes
     * 
     * @since 7.8.2
     */
    public function disable_intermediate_image_sizes__premium_only( $sizes, $image_meta ) {
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
            if ( ! $this->should_apply_image_upload_control__premium_only() ) {
                return $sizes;
            }
        ...
        $options = get_option( ASENHA_SLUG_U, array() );
        $disabled_image_sizes = isset( $options['disabled_image_sizes'] ) ? $options['disabled_image_sizes'] : array();

        $common_methods = new Common_Methods;
        $disabled_image_size_slugs = $common_methods->get_array_of_keys_with_true_value( $disabled_image_sizes );
        
        foreach( $sizes as $size_slug => $size_info ) {
            if ( in_array( $size_slug, $disabled_image_size_slugs ) ) {
                unset( $sizes[$size_slug] );
            }
        }
    ...
    /**
     * Whether Image Upload Control should process the current request.
     *
     * When the setting `image_upload_control_only_media_library_uploads` is enabled,
     * processing is limited to Media Library / wp.media uploads and REST media uploads
     * used by the block editor (`wp/v2/media`).
     *
     * @since 7.9.0
     *
     ...
     * @return bool
     */
    private function should_apply_image_upload_control__premium_only( $options = null ) {
        if ( null === $options ) {
            $options = get_option( ASENHA_SLUG_U, array() );
        }

        $only_media_library_uploads = isset( $options['image_upload_control_only_media_library_uploads'] )
            ? (bool) $options['image_upload_control_only_media_library_uploads']
            : false;

        if ( ! $only_media_library_uploads ) {
            return true;
        }

        return $this->is_media_library_upload_request__premium_only();
    }

    /**
     * Whether the upload request originated from an allowed wp-admin screen.
     *
     * @since 7.9.0
     *
     * @return bool
     */
    private function is_allowed_upload_referer__premium_only() {
        ...
        if ( empty( $admin_path ) || empty( $ref_path ) || 0 !== strpos( $ref_path, $admin_path ) ) {
            return false;
        }
        ...
        $ref_page = wp_basename( $ref_path );

        return in_array( $ref_page, $allowed_pages, true );
    }

    /**
     * Detect whether current request is a Media Library / block editor media upload.
     *
     * @since 7.9.0
     *
     * @return bool
     */
    private function is_media_library_upload_request__premium_only() {
        // AJAX uploader used by Media Library and wp.media (admin).
        if ( function_exists( 'wp_doing_ajax' ) && wp_doing_ajax() ) {
            if ( isset( $_REQUEST['action'] ) ) {
                $action = sanitize_key( wp_unslash( $_REQUEST['action'] ) );
                if ( 'upload-attachment' === $action ) {
                    return $this->is_allowed_upload_referer__premium_only();
                }
            }
        }

        // Admin async upload endpoint.
        if ( is_admin() ) {
            global $pagenow;
        ...
        // REST endpoint used by block editor and other clients: /wp/v2/media.
        if ( defined( 'REST_REQUEST' ) && REST_REQUEST ) {
            // If referrer is missing or not one of the allowed wp-admin pages, treat it as frontend.
            if ( ! $this->is_allowed_upload_referer__premium_only() ) {
                return false;
            }

            if ( isset( $_GET['rest_route'] ) ) {
                $rest_route = (string) wp_unslash( $_GET['rest_route'] );
                if ( 0 === strpos( $rest_route, '/wp/v2/media' ) ) {
                    return true;
                }
            }
            ...
            if ( '' !== $request_uri ) {
                $prefix = '/' . rest_get_url_prefix() . '/wp/v2/media';
                if ( false !== strpos( $request_uri, $prefix ) ) {
                    return true;
                }
            }
        }
