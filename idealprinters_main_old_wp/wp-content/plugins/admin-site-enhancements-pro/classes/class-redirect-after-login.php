...
class Redirect_After_Login {
    ...
    public function redirect_after_login( $username, $user ) {
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
            ...
            if ( 'separate_urls' == $redirect_after_login_type ) {

                $redirect_after_login_for_separate_role = isset( $options['redirect_after_login_for_separate_role'] ) ? $options['redirect_after_login_for_separate_role'] : array();

                $redirect_after_login_for_separate_slug = isset( $options['redirect_after_login_for_separate_slug'] ) ? $options['redirect_after_login_for_separate_slug'] : array();
                    ...
                    foreach ( $current_user_roles as $role ) {
                        foreach ( $redirect_after_login_for_separate_role as $role_slug => $custom_redirect ) {
                            if ( $role == $role_slug 
                                ...
                                $redirect_after_login_to_slug_raw = $redirect_after_login_for_separate_slug[$role_slug];
    ...
    public function get_redirect_relative_path( $redirect_after_login_to_slug_raw ) {        
        if ( ! empty( $redirect_after_login_to_slug_raw ) ) {
            ...
            if ( false !== strpos( $redirect_after_login_to_slug, '#' ) 
                || false !== strpos( $redirect_after_login_to_slug, '?' ) 
                || false !== strpos( $redirect_after_login_to_slug, '.php' ) 
                || false !== strpos( $redirect_after_login_to_slug, '.html' ) 
            ) {
                $slug_suffix = '';
            } else {
                $slug_suffix = '/';
