...
class Admin_Columns_Manager {
    ...
    public function enqueue_admin_columns_scripts__premium_only() {
        ...
        if ( isset( $_GET['for'] ) ) {
            $selected_post_type = sanitize_key( wp_unslash( $_GET['for'] ) );
        }

        $options = get_option( ASENHA_SLUG_U . '_extra', array() );
        $hscroll_setting_exists = (
            isset( $options['admin_columns_hscroll_disabled'] )
            && is_array( $options['admin_columns_hscroll_disabled'] )
            && array_key_exists( $selected_post_type, $options['admin_columns_hscroll_disabled'] )
        );
        ...
        wp_localize_script(
            ...
            array(
                ...
                'hScrollSettingExists' => $hscroll_setting_exists,
            )
        );
    }
    ...
    public function add_list_tables_scripts__premium_only( $hook_suffix ) {
        ...
        if ( 'edit.php' == $hook_suffix ) {
            ...
            $freeze_first_columns = 1;
            if (
                ! empty( $post_type )
                && isset( $options_extra['admin_columns_freeze_first_columns'][ $post_type ] )
            ) {
                $freeze_first_columns = absint( $options_extra['admin_columns_freeze_first_columns'][ $post_type ] );
            }
            if ( $freeze_first_columns > 5 ) {
                $freeze_first_columns = 5;
            }

            $freeze_header_row = false;
            if (
                ! empty( $post_type )
                && isset( $options_extra['admin_columns_freeze_header_row'][ $post_type ] )
            ) {
                $freeze_header_row = (bool) $options_extra['admin_columns_freeze_header_row'][ $post_type ];
            }

            wp_localize_script(
                'asenha-list-tables-organize-columns',
                'asenhaListTablesOrganizeColumns',
                array(
                    'freezeFirstColumns' => $freeze_first_columns,
    ...
    public function render_admin_columns_organizer_settings_page__premium_only() {
        ...
        if ( $hscroll_setting_exists ) {
            $is_horizontal_scroll_disabled = (bool) $options['admin_columns_hscroll_disabled'][ $selected_post_type ];
        } else {
            // If no saved value exists yet, default to disabling horizontal scroll when all columns are Auto width.
            $is_horizontal_scroll_disabled = $this->are_all_active_columns_auto_width__premium_only( $selected_post_type, $options );
        }

        // Freeze the first columns (checkbox column + N data columns).
        // Value represents the number of data columns (excludes checkbox column). 0 = No columns.
        $freeze_first_columns = 1;
        if (
            isset( $options['admin_columns_freeze_first_columns'] )
            && is_array( $options['admin_columns_freeze_first_columns'] )
            && array_key_exists( $selected_post_type, $options['admin_columns_freeze_first_columns'] )
        ) {
            $freeze_first_columns = absint( $options['admin_columns_freeze_first_columns'][ $selected_post_type ] );
        }
        ...
        // Freeze the column header row (sticky on vertical scroll), per post type.
        $freeze_header_row = false;
        if (
            isset( $options['admin_columns_freeze_header_row'] )
            && is_array( $options['admin_columns_freeze_header_row'] )
            && array_key_exists( $selected_post_type, $options['admin_columns_freeze_header_row'] )
        ) {
            $freeze_header_row = (bool) $options['admin_columns_freeze_header_row'][ $selected_post_type ];
        }
                ...
                <div class="admin-columns-current"><!-- Start of ACTIVE columns -->
                    ...
                    <div class="additional-settings">
                        ...
                        <div class="freeze-settings">
                            <div class="additional-setting">
                                <input type="checkbox" id="freeze-header-row" name="freeze-header-row" <?php checked( $freeze_header_row, true ); ?> class="freeze-header-row-checkbox" >
                                <label for="freeze-header-row"><?php echo esc_html__( 'Freeze the column header row', 'admin-site-enhancements' ); ?></label>
                            </div>
                            <div class="additional-setting additional-setting-freeze-columns<?php echo $is_horizontal_scroll_disabled ? ' is-hidden' : ''; ?>">
                                <label for="freeze-first-columns"><span class="freeze-settings-separator">|</span><?php echo esc_html__( 'Freeze the first columns', 'admin-site-enhancements' ); ?></label>
                                <select id="freeze-first-columns" name="freeze-first-columns">
                                    <option value="0" <?php selected( $freeze_first_columns, 0 ); ?>><?php echo esc_html__( 'No columns', 'admin-site-enhancements' ); ?></option>
                                    <?php for ( $i = 1; $i <= 5; $i++ ) : ?>
                                        <option value="<?php echo esc_attr( $i ); ?>" <?php selected( $freeze_first_columns, $i ); ?>>
                                            <?php
                                            echo esc_html(
                                                sprintf(
                                                    /* translators: %d: number of columns. */
                                                    _n( '%d column', '%d columns', $i, 'admin-site-enhancements' ),
                                                    $i
                                                )
                                            );
                                            ?>
                ...
                <div class="admin-columns-available"><!-- Start of AVAILABLE columns -->
                    ...
                    <div class="extra-columns-container<?php if ( empty( $extra_columns ) ) : ?> no-columns<?php endif; ?>">
                        ...
                        <?php if ( ! empty( $extra_columns ) ) : ?>
                            <?php
                            // Get sortable columns registered by plugins for this post type
                            $plugin_sortable_columns = $this->get_plugin_sortable_columns__premium_only( $selected_post_type );
                            ?>
                            <?php foreach ( $extra_columns as $column_key => $column_data ): ?>
                                ...
                                $is_extra_column = 'yes';
                                $is_custom_field = 'no';
                                // Check if this extra column is registered as sortable by a plugin
                                $is_sortable = in_array( $column_key, $plugin_sortable_columns ) ? 'yes' : 'maybe';
                                $is_the_default_sort = ( isset( $column_data['is_the_default_sort'] ) ) ? $column_data['is_the_default_sort'] : false;
                                if ( $is_the_default_sort ) {
                                    $sort_order_display = ' style="display:flex;"';
                                } else {
    ...
    public function check_column_sortability__premium_only( $selected_post_type, $column_key, $column_info ) {
        ...
        if ( in_array( $column_key, $custom_fields_keys ) ) {
            ...
            // For extra columns, check if they are registered as sortable by a plugin
            $plugin_sortable_columns = $this->get_plugin_sortable_columns__premium_only( $selected_post_type );
            if ( in_array( $column_key, $plugin_sortable_columns ) ) {
                $is_sortable = 'yes';
            } else {
                $is_sortable = 'maybe';
            }
    ...
    /**
     * Get sortable columns registered by plugins via the manage_edit-{post_type}_sortable_columns filter
     *
     * @since 7.7.0
     * @param string $post_type The post type to get sortable columns for.
     * @return array Array of sortable column keys registered by plugins.
     */
    public function get_plugin_sortable_columns__premium_only( $post_type ) {
        // Get sortable columns registered by plugins via the WordPress filter
        // We pass an empty array to get only columns added by plugins
        $plugin_sortable_columns = apply_filters( 'manage_edit-' . $post_type . '_sortable_columns', array() );
        
        // Return the column keys
        return array_keys( $plugin_sortable_columns );
    }
    ...
    /**
     * Check whether all configured active columns are effectively auto width.
     *
     * A column is considered fixed width only if:
     * - width_type is 'px' or '%', AND
     * - width is a positive number.
     *
     * Otherwise (including empty width values), the effective width is treated as Auto.
     *
     * @since 8.2.4
     *
     * @param string $post_type Post type slug.
     * @param array  $options   Options stored in ASENHA_SLUG_U . '_extra'.
     * @return bool True when all columns are auto width.
     */
    private function are_all_active_columns_auto_width__premium_only( $post_type, $options ) {
        $post_type = sanitize_key( $post_type );

        $columns = array();
        if ( isset( $options['admin_columns'][ $post_type ] ) && is_array( $options['admin_columns'][ $post_type ] ) ) {
            $columns = $options['admin_columns'][ $post_type ];
        }
        ...
        foreach ( $columns as $column_data ) {
            ...
            $width_type = isset( $column_data['width_type'] ) ? (string) $column_data['width_type'] : 'auto';
            $width      = isset( $column_data['width'] ) ? $column_data['width'] : '';

            $width_type  = strtolower( trim( $width_type ) );
            $width_float = is_numeric( $width ) ? (float) $width : 0;

            if ( in_array( $width_type, array( 'px', '%' ), true ) && $width_float > 0 ) {
                return false;
            }
        }
    ...
    public function add_additional_elements__premium_only() {
        ...
        if ( 'posts' == $typenow ) {
            $typenow = 'post'; // We need this in the singular noun.
        }
        ...
        $is_all_columns_auto_width = $this->are_all_active_columns_auto_width__premium_only( $typenow, $options );
        $is_horizontal_scroll_disabled_effective = ( $is_horizontal_scroll_disabled || $is_all_columns_auto_width );
        ...
        // List tables of pages, posts and CPTs. Administrators only.
        if ( 'edit.php' == $pagenow 
            && ! $is_horizontal_scroll_disabled_effective
        ) {
    ...
    /**
     * Register admin columns hooks during Quick Edit save (inline-save) AJAX request.
     *
     * WordPress runs Quick Edit saving via admin-ajax.php?action=inline-save. In that context,
     * $pagenow is not 'edit.php', so hooks registered in render_columns__premium_only() are not
     * added and the returned row markup can be generated using default columns. That causes
     * the injected row to have a different set/order of <td> cells than the current list table,
     * resulting in missing/shifted cells.
     *
     * This method registers the same hooks for the specific post type being quick-edited,
     * before WordPress core runs wp_ajax_inline_save().
     *
     * @since 8.2.4
     * @return void
     */
    public function register_columns_hooks_for_inline_save__premium_only() {

        if ( ! wp_doing_ajax() ) {
            return;
        }

        if ( ! isset( $_REQUEST['action'] ) || 'inline-save' !== sanitize_key( wp_unslash( $_REQUEST['action'] ) ) ) {
            return;
        }
        ...
        // Keep behavior consistent with render_columns__premium_only().
        $excluded_post_types = array(
            'attachment',
            'wp_navigation',
        );
        ...
        // Several callbacks rely on $typenow.
        global $typenow;
        $typenow = $post_type;

        add_filter( 'manage_edit-' . $post_type . '_columns', array( $this, 'custom_columns_order__premium_only' ), PHP_INT_MAX );
        add_action( 'manage_' . $post_type . '_posts_custom_column', array( $this, 'custom_columns_content__premium_only' ), PHP_INT_MAX, 2 );
        add_action( 'manage_edit-' . $post_type . '_sortable_columns', array( $this, 'sortable_columns__premium_only' ), PHP_INT_MAX );
        add_filter( 'posts_clauses', array( $this, 'make_taxonomy_columns_sortable__premium_only' ), 10, 2 );
    }
        ...
        if ( ! isset( $options['admin_columns'][$typenow] ) ) {
            ...
            foreach( $custom_columns as $custom_column_key => $custom_column_data ) {
                if ( isset( $custom_column_data['use_original_title'] ) && $custom_column_data['use_original_title'] ) {
                    $title = isset( $columns[$custom_column_key] ) ? $columns[$custom_column_key] : '';
                } else {
    ...
    public function apply_column_sort__premium_only( $query ) {
        ...
        foreach( $sortable_column_keys as $column_key ) {
            ...
            // Apply sorting for the featured image column
            if ( 'featured_image' == $query->get( 'orderby' ) ) {
                // https://plugins.trac.wordpress.org/browser/featured-image-admin-thumb-fiat/tags/1.6.1/admin/class-featured-image-admin-thumb-admin.php#L439
                // When sorting ascending, posts with a featured image will be shown
                // When sorting descending, posts without a featured image will be shown
                    ...
                    foreach ( $current_post_type_columns as $column_key => $column ) {
                        foreach ( $column as $key => $value ) {
                            if ( 'is_the_default_sort' == $key ) {
                                if ( true === $value ) {
                                    if ( isset( $current_post_type_columns[$column_key]['sort_order'] ) ) {
    ...
    public function make_taxonomy_columns_sortable__premium_only( $clauses, $wp_query ) {
        ...
        if ( isset( $wp_query->query['orderby'] ) || $wp_query->get( 'orderby' ) ) {
            $orderby = $wp_query->get( 'orderby' );
            if ( empty( $orderby ) && isset( $wp_query->query['orderby'] ) ) {
            ...
            // Sort Pages by template title (nice name).
            if ( 'template' === $orderby && 'page' === $post_type ) {
                $template_title_map = $this->get_template_title_map__premium_only( $post_type );

                if ( false === strpos( (string) $clauses['join'], 'asenha_tpl_pm' ) ) {
                    $clauses['join'] .= " LEFT JOIN {$wpdb->postmeta} AS asenha_tpl_pm ON ({$wpdb->posts}.ID = asenha_tpl_pm.post_id AND asenha_tpl_pm.meta_key = '_wp_page_template') ";
                }
                ...
                // Always normalize missing/empty values.
                $case_parts[] = "WHEN COALESCE(asenha_tpl_pm.meta_value,'') = '' THEN '" . esc_sql( $template_title_map[''] ?? __( 'Default template', 'admin-site-enhancements' ) ) . "'";
                $case_parts[] = "WHEN COALESCE(asenha_tpl_pm.meta_value,'') = 'default' THEN '" . esc_sql( $template_title_map['default'] ?? ( $template_title_map[''] ?? __( 'Default template', 'admin-site-enhancements' ) ) ) . "'";

                foreach ( $template_title_map as $template_identifier => $template_title ) {
                    if ( '' === $template_identifier || 'default' === $template_identifier ) {
                        continue;
                    }

                    if ( ! is_string( $template_title ) || '' === $template_title ) {
                        continue;
                    }

                    $case_parts[] = "WHEN asenha_tpl_pm.meta_value = '" . esc_sql( $template_identifier ) . "' THEN '" . esc_sql( $template_title ) . "'";
                }

                $case_sql = 'CASE ' . implode( ' ', $case_parts ) . " ELSE COALESCE(asenha_tpl_pm.meta_value,'') END";

                // Note: $clauses['orderby'] expects only the expression, without the ORDER BY keyword.
                $clauses['orderby'] = $case_sql . ' ' . $order . ", {$wpdb->posts}.ID " . $order;
    ...
    /**
     * Get a mapping of stored page template identifiers to their display titles.
     *
     * Used for sorting (so we sort by the same “nice name” we display).
     *
     * @since 8.2.3
     *
     * @param string $post_type Post type.
     * @return array<string,string> Map of template identifier => template title.
     */
    private function get_template_title_map__premium_only( $post_type ) {
        $post_type = is_string( $post_type ) ? sanitize_key( $post_type ) : '';
        ...
        $default_label = __( 'Default template', 'admin-site-enhancements' );

        $is_block_theme = false;
        if ( function_exists( 'wp_is_block_theme' ) ) {
            $is_block_theme = (bool) wp_is_block_theme();
        } elseif ( function_exists( 'current_theme_supports' ) ) {
            $is_block_theme = (bool) current_theme_supports( 'block-templates' );
        }

        $default_title = $default_label;
        if ( $is_block_theme ) {
            $block_default_title = $this->get_block_template_title_by_slug__premium_only( 'page' );
            if ( ! empty( $block_default_title ) ) {
                $default_title = $block_default_title;
            }
        }
        ...
        // Classic templates: file path => template title.
        $theme = wp_get_theme();
        if ( is_object( $theme ) && method_exists( $theme, 'get_page_templates' ) ) {
            ...
            if ( is_array( $page_templates ) ) {
                foreach ( $page_templates as $template_file => $template_title ) {
                    if ( is_string( $template_file ) && is_string( $template_title ) ) {
                        $map[ $template_file ] = $template_title;
                    }
                }
            }
        }

        // Block templates: slug => template title.
        if ( function_exists( 'get_block_templates' ) ) {
            $templates = get_block_templates( array(), 'wp_template' );
            if ( is_array( $templates ) ) {
                foreach ( $templates as $template ) {
                    if ( is_object( $template )
                        && isset( $template->slug, $template->title )
                        && is_string( $template->slug )
                        && is_string( $template->title )
                    ) {
                        $map[ $template->slug ] = $template->title;
                    }
                }
            }
        }
    ...
    public function get_default_columns( $post_type ) {
        ...
        // Pages can have a template assigned (classic templates and/or block theme templates).
        if ( 'page' === $post_type ) {
            $available_columns['template'] = __( 'Template', 'admin-site-enhancements' );
        }
    ...
    public function render_default_columns__premium_only( $column_name, $post_id, $typenow, $format ) {
        ...
        if ( 'template' === $column_name ) {
            return esc_html( $this->get_template_display_title__premium_only( (int) $post_id, (string) $typenow ) );
        }
    ...
    /**
     * Get a human-readable title for the template assigned to a post.
     *
     * For classic themes, this is the PHP page template name.
     * For block themes, this is the block template title (e.g. "Pages").
     *
     * @since 8.2.3
     *
     ...
     */
    public function get_template_display_title__premium_only( $post_id, $post_type ) {
        $default_label = __( 'Default template', 'admin-site-enhancements' );

        $template_identifier = '';
        if ( function_exists( 'get_page_template_slug' ) ) {
            $template_identifier = get_page_template_slug( $post_id );
        }
        if ( empty( $template_identifier ) ) {
            $template_identifier = get_post_meta( $post_id, '_wp_page_template', true );
        }

        $template_identifier = is_string( $template_identifier ) ? trim( $template_identifier ) : '';
        ...
        // In block themes, Pages commonly default to the 'page' template (shown as "Pages" in the editor).
        if ( $is_block_theme && ( '' === $template_identifier || 'default' === $template_identifier ) && 'page' === $post_type ) {
            $block_default_title = $this->get_block_template_title_by_slug__premium_only( 'page' );
            if ( ! empty( $block_default_title ) ) {
                return $block_default_title;
            }
        ...
        // Classic page templates (PHP files).
        $theme = wp_get_theme();
        if ( is_object( $theme ) && method_exists( $theme, 'get_page_templates' ) ) {
            $page_templates = $theme->get_page_templates( null, $post_type );
            if ( is_array( $page_templates ) && isset( $page_templates[ $template_identifier ] ) ) {
                return $page_templates[ $template_identifier ];
            }
        }

        // Block templates (slugs).
        $block_title = $this->get_block_template_title_by_slug__premium_only( $template_identifier );
        if ( ! empty( $block_title ) ) {
            return $block_title;
        }

        // Fallback: humanize the stored identifier.
        return $this->humanize_template_identifier__premium_only( $template_identifier );
    }

    /**
     * Resolve a block template title by its slug.
     *
     * @since 8.2.3
     *
     * @param string $slug Block template slug.
     * @return string Block template title or empty string.
     */
    private function get_block_template_title_by_slug__premium_only( $slug ) {
        ...
        // If this looks like a classic PHP template path, skip block-template lookup.
        if ( false !== strpos( $slug, '.php' ) || false !== strpos( $slug, '/' ) ) {
            return '';
        }

        $theme_stylesheet = function_exists( 'get_stylesheet' ) ? get_stylesheet() : '';

        if ( function_exists( 'get_block_template' ) && ! empty( $theme_stylesheet ) ) {
            $template_id = $theme_stylesheet . '//' . $slug;
            $template    = get_block_template( $template_id, 'wp_template' );
            if ( is_object( $template ) && isset( $template->title ) && is_string( $template->title ) ) {
    ...
    /**
     * Humanize a template identifier into a readable label.
     *
     * @since 8.2.3
     *
     * @param string $identifier Template identifier (file path or slug).
     * @return string Humanized label.
     */
    private function humanize_template_identifier__premium_only( $identifier ) {
    ...
    public function render_ase_cf_columns__premium_only( $cf_type = 'text', $cf_value = '', $column_name = '', $post_id = false, $typenow = '', $format = array() ) {
        ...
        if ( 'datetime' == $cf_type ) {
            $raw_datetime = is_string( $cf_value ) ? trim( $cf_value ) : '';
            if ( '' === $raw_datetime ) {
                return '';
            }

            // Formatting will use field settings in the custom field group
            return $raw_datetime;
        }
                        ...
                        foreach ( $repeater_item as $scf_key => $scf_value ) {
                            if ( isset( $custom_fields[$scf_key] ) ) {
                                ...
                                if ( 'repeater' != $scf_type ) {
                                    ...
                                    // Ensure wp_kses() never receives a null subject (PHP 8.1+ deprecation via preg_replace()).
                                    if ( is_array( $render ) || is_null( $render ) ) {
                                        $render = '';
                                    } else {
                                        ...
                                        foreach ( $scf_value as $sub_scf_key => $sub_scf_value ) {
                                            ...
                                            foreach ( $sub_scf_value as $key => $value ) {
                                                if ( isset( $custom_fields[$key] ) ) {
                                                    ...
                                                    // Ensure wp_kses_post() never receives a null subject (PHP 8.1+ deprecation via preg_replace()).
                                                    if ( is_array( $render ) || is_null( $render ) ) {
                                                        $render = '';
                                                    } else {
                                                        $render = (string) $render;
    ...
    public function save_columns_order__premium_only() {
        ...
        if ( isset( $_REQUEST ) ) {
            if ( check_ajax_referer( 'admin-columns-nonce', 'nonce', false ) ) {
                ...
                // Freeze first columns (checkbox + N data columns). Value is N (excludes checkbox).
                $freeze_first_columns = 1;
                if ( isset( $_REQUEST['freeze_first_columns'] ) ) {
                    $freeze_first_columns = absint( $_REQUEST['freeze_first_columns'] );
                }
                if ( $freeze_first_columns > 5 ) {
                    $freeze_first_columns = 5;
                }
                $options['admin_columns_freeze_first_columns'][ $post_type ] = $freeze_first_columns;

                // Freeze the column header row (sticky on vertical scroll), per post type.
                $freeze_header_row = ( isset( $_REQUEST['freeze_header_row'] ) && 'true' === $_REQUEST['freeze_header_row'] ) ? true : false;
                $options['admin_columns_freeze_header_row'][ $post_type ] = $freeze_header_row;
     ...
     *
     * Note: This method is kept for backward reference, but is no longer hooked to
     * the core 'wp_ajax_inline-save' action in bootstrap.php.
     * 
