...
class Disable_Comments {
    ...
    public function disable_comments_for_post_types_edit() {
        ...
        foreach ( $disable_comments_for as $post_type_slug => $is_post_type_checked ) {
            if ( 'only-on' == $disable_comments_type && $is_post_type_checked 
                ...
                || 'all-post-types' == $disable_comments_type
            ) {
    ...
    public function is_commenting_disabled_for_post_type( $post_type ) {
        ...
        if ( array_key_exists( 'disable_comments', $options ) && $options['disable_comments'] ) {
            if ( array_key_exists( 'disable_comments_for', $options ) && ! empty( $options['disable_comments_for'] ) )  {
                ...
                $disable_comments_for = $common_methods->get_array_of_keys_with_true_value( $disable_comments_for ); // array of post type slugs
                ...
                if ( 'only-on' == $disable_comments_type && in_array( $post_type, $disable_comments_for ) 
    ...
    /**
     * Remove the Comments admin menu item when comments are disabled on all post types.
     *
     * @since 7.11.0
     */
    public function maybe_remove_comments_admin_menu__premium_only() {
        $options = get_option( ASENHA_SLUG_U, array() );
        $disable_comments_type = isset( $options['disable_comments_type'] ) ? $options['disable_comments_type'] : 'only-on';

        if ( 'all-post-types' !== $disable_comments_type ) {
            return;
        }
    ...
    /**
     * Remove the Comments counter/link in the admin bar when comments are disabled on all post types.
     *
     * @since 7.11.0
     *
     * @param WP_Admin_Bar $wp_admin_bar Admin bar object.
     */
    public function maybe_remove_comments_admin_bar_menu__premium_only( $wp_admin_bar ) {
        $options = get_option( ASENHA_SLUG_U, array() );
        $disable_comments_type = isset( $options['disable_comments_type'] ) ? $options['disable_comments_type'] : 'only-on';
        ...
        // Prevent core comments node from being added.
        remove_action( 'admin_bar_menu', 'wp_admin_bar_comments_menu', 60 );

        // Defensive: if it's already been added by a custom callback, remove the node.
