...
class Enhance_List_Tables {
    ...
    public function add_excerpt( $column_name, $id ) {
        ...
        if ( 'asenha-excerpt' === $column_name ) {
            ...
            $excerpt = wp_strip_all_tags( get_the_excerpt( $id ) ); // about 310 characters
    ...
    public function show_last_modified_column() {
        foreach ( get_post_types() as $post_type ) {
            add_filter( 'manage_' . $post_type . '_posts_columns', [ $this, 'add_last_modified_column' ], 10, 1 );
            add_action( 'manage_' . $post_type . '_posts_custom_column', [ $this, 'show_last_modified_datetime' ], 10, 2 );
            add_filter( 'manage_edit-' . $post_type . '_sortable_columns', [ $this, 'make_last_modified_column_sortable' ], 10, 1 );
        }
    }
    ...
    public function show_last_modified_datetime( $column_name, $id ) {        
        if ( 'asenha-last-modified' == $column_name ) {
            ...
            if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
                ...
                // Get column settings if available
                if ( isset( $options['admin_columns'] ) ) {
                    if ( isset( $options['admin_columns'][$post_type]['asenha-last-modified'] ) ) {
                        $column_settings = $options['admin_columns'][$post_type]['asenha-last-modified'];
                    }
                }                    
                
                $format_type = isset( $column_settings['format_type'] ) ? $column_settings['format_type'] : 'default'; 
                ...
                if ( 'date_time' == $format_type ) {
                    ...
                    echo '<span class="last-modified-timestamp">' . wp_date( $date_time_format, $modified_date_time_unix ) . '</span>';  
                } else {
     ...
     * @since 6.9.11
     */
    public function make_file_size_columns_sortable__premium_only( $sortable_columns ) {
        $sortable_columns['asenha-file-size'] = 'file-size';
        return $sortable_columns;
    }
    
    /**
     * Enable sorting of media files by file size
     * 
     * @since 6.9.11
     */
    public function sort_media_by_file_size__premium_only( $query ) {
        if ( ! is_admin() || ! $query->is_main_query() ) {
            return;
        }

        if ( 'file-size' === $query->get('orderby') ) {
    ...
    public function show_id_in_action_row() {
        ...
        add_filter( 'page_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
        add_filter( 'post_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
        add_filter( 'cat_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
        add_filter( 'tag_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
        add_filter( 'media_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
        add_filter( 'comment_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
        add_filter( 'user_row_actions', array( $this, 'add_id_in_action_row' ), 99, 2 );
    ...
    public function hide_date_column() {
        foreach ( get_post_types() as $post_type ) {
            add_filter( 'manage_' . $post_type . '_posts_columns', [ $this, 'remove_date_column' ], 10, 1 );
        }
    }
