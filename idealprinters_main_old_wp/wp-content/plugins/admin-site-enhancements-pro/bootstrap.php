...
class Admin_Site_Enhancements {
	...
	private function __construct() {
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'two_factor_authentication', $options ) && $options['two_factor_authentication'] ) {
				require_once ASENHA_PATH . 'includes/premium/two-factor-authentication/two-factor-authentication.php';
			}
		}
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'form_builder', $options ) && $options['form_builder'] ) {
				...
				// Export Form Builder forms
				add_action( 'admin_init', 'asenha_forms_export__premium_only' );
				// Import Form Builder forms
				add_action( 'admin_init', 'asenha_forms_import__premium_only' );
				// Export Form Builder style templates
				add_action( 'admin_init', 'asenha_style_templates_export__premium_only' );
				// Import Form Builder style templates
				add_action( 'admin_init', 'asenha_style_templates_import__premium_only' );
				// Export Form Builder entries
				add_action( 'admin_init', 'asenha_entries_export__premium_only' );
				// Get Form Builder forms for export (via AJAX)
				add_action( 'wp_ajax_get_forms_for_export', 'asenha_get_forms_for_export__premium_only' );
				// Get Form Builder style templates for export (via AJAX)
				add_action( 'wp_ajax_get_style_templates_for_export', 'asenha_get_style_templates_for_export__premium_only' );
				// Get Form Builder form fields for entries export (via AJAX)
				add_action( 'wp_ajax_get_form_fields_for_entry_export', 'asenha_get_form_fields_for_entry_export__premium_only' );
			}
