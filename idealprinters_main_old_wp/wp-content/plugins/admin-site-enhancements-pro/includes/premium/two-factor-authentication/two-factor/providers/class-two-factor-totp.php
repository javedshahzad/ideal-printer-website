...
class Two_Factor_Totp extends Two_Factor_Provider {
	...
	public function get_label() {
		return _x( 'Authenticator App', 'Provider Label', 'admin-site-enhancements' );
	}
	...
	public function get_alternative_provider_label() {
		return __( 'Use your authenticator app', 'admin-site-enhancements' ) . ' »';
	}
	...
	public function rest_delete_totp( $request ) {
		...
		if ( ! Two_Factor_Core::disable_provider_for_user( $user_id, 'Two_Factor_Totp' ) ) {
			return new WP_Error( 'db_error', __( 'Unable to disable TOTP provider for this user.', 'admin-site-enhancements' ), array( 'status' => 500 ) );
		}
	...
	public function rest_setup_totp( $request ) {
		...
		if ( ! $this->is_valid_key( $key ) ) {
			return new WP_Error( 'invalid_key', __( 'Invalid Two Factor Authentication secret key.', 'admin-site-enhancements' ), array( 'status' => 400 ) );
		}

		if ( ! $this->is_valid_authcode( $key, $code ) ) {
			return new WP_Error( 'invalid_key_code', __( 'Invalid Two Factor Authentication code.', 'admin-site-enhancements' ), array( 'status' => 400 ) );
		}

		if ( ! $this->set_user_totp_key( $user_id, $key ) ) {
			return new WP_Error( 'db_error', __( 'Unable to save Two Factor Authentication code. Please re-scan the QR code and enter the code provided by your application.', 'admin-site-enhancements' ), array( 'status' => 500 ) );
		}

		if ( $request->get_param( 'enable_provider' ) && ! Two_Factor_Core::enable_provider_for_user( $user_id, 'Two_Factor_Totp' ) ) {
			return new WP_Error( 'db_error', __( 'Unable to enable TOTP provider for this user.', 'admin-site-enhancements' ), array( 'status' => 500 ) );
		}
	...
	public function user_two_factor_options( $user ) {
		...
		if ( empty( $key ) ) :
			...
			?>
			<p>
				<?php esc_html_e( 'Please scan the QR code or manually copy the shared secret key from below to your Authenticator app:', 'admin-site-enhancements' ); ?>
			</p>
			<p id="two-factor-qr-code">
				<a href="<?php echo esc_url( $totp_url ); ?>">
					<?php esc_html_e( 'Loading…', 'admin-site-enhancements' ); ?>
				...
				(function(){
					var qr_generator = function() {
						...
						*/
						...
						svg.role = 'image';
						svg.ariaLabel = <?php echo wp_json_encode( __( 'Authenticator App QR Code', 'admin-site-enhancements' ) ); ?>;
			...
			<p>
				<?php esc_html_e( 'Shared secret key:', 'admin-site-enhancements' ); ?> <code><?php echo esc_html( $key ); ?></code>
			</p>
			<hr />
			<p>
				<?php esc_html_e( 'Enter the code generated by the Authenticator app to complete the setup:', 'admin-site-enhancements' ); ?>
			</p>
			<p>
				...
				<label for="two-factor-totp-authcode">
					<?php esc_html_e( 'Authentication Code:', 'admin-site-enhancements' ); ?>
					<?php
						/* translators: Example auth code. */
						$placeholder = sprintf( __( 'eg. %s', 'admin-site-enhancements' ), '123456' );
					?>
				...
				<button type="submit" class="button totp-submit asenha-2fa-action-btn asenha-2fa-totp-verify-btn" name="two-factor-totp-submit" value="1">
		...
		<?php else : ?>
			<p class="success">
				<?php esc_html_e( 'An authenticator app is currently configured. You will need to re-scan the QR code on all devices if reset.', 'admin-site-enhancements' ); ?>
			</p>
			<p>
				<button type="button" class="button button-secondary reset-totp-key hide-if-no-js asenha-2fa-action-btn asenha-2fa-totp-reset-btn">
					<span class="asenha-2fa-btn-text"><?php esc_html_e( 'Reset authenticator app', 'admin-site-enhancements' ); ?></span>
					<span class="spinner asenha-2fa-btn-spinner" style="display:none;"></span>
					<span class="asenha-2fa-btn-checkmark dashicons dashicons-yes" style="display:none;"></span>
				</button>
				<script>
	...
	public function authentication_page( $user ) {
		...
		<p class="two-factor-prompt">
			<?php esc_html_e( 'Enter the code generated by your authenticator app.', 'admin-site-enhancements' ); ?>
		</p>
		<p>
			<label for="authcode"><?php esc_html_e( 'Authentication Code:', 'admin-site-enhancements' ); ?></label>
		...
		<p class="submit">
			<button type="submit" class="button asenha-2fa-action-btn asenha-2fa-totp-login-verify-btn">
				<span class="asenha-2fa-btn-text"><?php esc_html_e( 'Verify', 'admin-site-enhancements' ); ?></span>
				<span class="spinner asenha-2fa-btn-spinner" style="display:none;"></span>
				<span class="asenha-2fa-btn-checkmark dashicons dashicons-yes" style="display:none;"></span>
			</button>
