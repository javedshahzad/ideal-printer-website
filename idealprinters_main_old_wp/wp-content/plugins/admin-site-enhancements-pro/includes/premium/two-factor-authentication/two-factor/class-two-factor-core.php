...
class Two_Factor_Core {
	...
	public static function filter_authenticate( $user ) {
		if ( $user instanceof WP_User && self::is_api_request() && self::is_user_using_two_factor( $user->ID ) && ! self::is_user_api_login_enabled( $user->ID ) ) {
			return new WP_Error(
				...
				__( 'Error: API login for user disabled.', 'admin-site-enhancements' )
			);
		}
	...
	public static function show_two_factor_login( $user ) {
		...
		if ( ! $login_nonce ) {
			wp_die( esc_html__( 'Failed to create a login nonce.', 'admin-site-enhancements' ) );
		}
	...
	public static function maybe_show_last_login_failure_notice( $user ) {
		...
		if ( $last_failed_two_factor_login ) {
			...
			printf(
				/* translators: %1$s: failed login count, %2$s: last failed login time. */
				_n(
	...
	public static function maybe_show_reset_password_notice( $errors ) {
		...
		$errors->add(
			...
			sprintf(
					/* translators: %s: link to password reset page. */
					__( 'Your password was reset because of too many failed Two Factor attempts. You will need to <a href="%s">create a new password</a> to regain access. Please check your email for more information.', 'admin-site-enhancements' ),
	...
	public static function login_html( $user, $login_nonce, $redirect_to, $error_msg = '', $provider = null, $action = 'validate_2fa' ) {
		...
		if ( ! $provider ) {
			wp_die( __( 'Cheatin&#8217; uh?', 'admin-site-enhancements' ) );
		}
		...
		if ( $backup_providers ) :
			...
			<div class="backup-methods-wrap">
				<p>
					<?php esc_html_e( 'Having problems?', 'admin-site-enhancements' ); ?>
				</p>
				<ul>
					<?php
			...
			(function() {
				...
				const getVerifyButton = () => {
					...
					return form.querySelector(
						'.asenha-2fa-verify-btn,' +
						'.asenha-2fa-recovery-verify-btn,' +
						'.asenha-2fa-totp-login-verify-btn,' +
						'button[type=\"submit\"],' +
						'input[type=\"submit\"]'
					);
				};
				...
				if ( inputEl ) {
					...
					const submitWithSpinner = () => {
						...
						if ( window.asenhaTwoFactorButtons && window.jQuery && verifyButton ) {
							window.asenhaTwoFactorButtons.showSpinner( window.jQuery( verifyButton ) );
						}

						if ( undefined !== form.requestSubmit ) {
							form.requestSubmit( verifyButton || undefined );
						} else {
							form.submit();
						}
					};
	...
	public static function rest_api_can_edit_user_and_update_two_factor_options( $user_id ) {
		...
		if ( ! self::current_user_can_update_two_factor_options( 'save' ) ) {
			return new WP_Error( 'revalidation_required', __( 'Two Factor Revalidation required.', 'admin-site-enhancements' ) );
		}
	...
	public static function _login_form_validate_2fa( $user, $nonce = '', $provider = '', $redirect_to = '', $is_post_request = false ) {
		...
		if ( $interim_login ) {
			...
			$message       = '<p class="message">' . __( 'You have logged in successfully.', 'admin-site-enhancements' ) . '</p>';
	...
	public static function _login_form_revalidate_2fa( $nonce = '', $provider = '', $redirect_to = '', $is_post_request = false ) {
		...
		if ( $interim_login ) {
			$message       = '<p class="message">' . __( 'You have revalidated successfully.', 'admin-site-enhancements' ) . '</p>';
			$interim_login = 'success'; // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
			login_header( '', $message );
			?>
			</div>
	...
	public static function process_provider( $provider, $user, $is_post_request ) {
		if ( ! $provider ) {
			return new WP_Error(
				...
				__( 'Cheatin&#8217; uh?', 'admin-site-enhancements' )
			);
		}
		...
		if ( true === self::is_user_rate_limited( $user ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'ERROR: Too many invalid codes, you can try again in %s. This limit protects your account against automated attacks.', 'admin-site-enhancements' ),
		...
		if ( true !== $provider->validate_authentication( $user ) ) {
			...
			return new WP_Error(
				'two_factor_invalid',
				__( 'ERROR: Invalid code.', 'admin-site-enhancements' )
			);
		}
	...
	public static function should_reset_password( $user_id ) {
		 ...
		 * like TOTP and recovery codes -- are very weak on their own, so it's not safe to give
	...
	public static function show_password_reset_error() {
		$error = new WP_Error(
			...
			sprintf(
				...
				__( 'There have been too many failed two-factor authentication attempts, which often indicates that the password has been compromised. The password has been reset in order to protect the account.', 'admin-site-enhancements' ),
				__( 'If you are the owner of this account, please check your email for instructions on regaining access.', 'admin-site-enhancements' )
			)
		);

		login_header( __( 'Password Reset', 'admin-site-enhancements' ), '', $error );
		login_footer();
	}
	...
	public static function filter_manage_users_columns( array $columns ) {
		$columns['two-factor'] = __( 'Two-Factor', 'admin-site-enhancements' );
		return $columns;
	}
	...
	public static function manage_users_custom_column( $output, $column_name, $user_id ) {
		...
		if ( ! self::is_user_using_two_factor( $user_id ) ) {
			return sprintf( '<span class="dashicons-before dashicons-no-alt">%s</span>', esc_html__( 'Disabled', 'admin-site-enhancements' ) );
		} else {
	...
	public static function user_two_factor_options( $user ) {
		/**
		 * Filter whether the Two-Factor options UI should be shown for the user.
		 *
		 * @since 8.2.4
		 *
		 * @param bool    $show Whether to show the Two-Factor options UI.
		 * @param WP_User $user The user being edited.
		 */
		if ( ! apply_filters( 'asenha_two_factor_show_user_options', true, $user ) ) {
			return;
		}
		...
		if ( ! $show_2fa_options ) {
			...
			$notices['warning two-factor-warning-revalidate-session'] = sprintf(
				esc_html__( 'To update your Two-Factor options, you must first revalidate your session.', 'admin-site-enhancements' ) . ' %s',
				sprintf(
					'<a class="button asenha-2fa-action-btn asenha-2fa-revalidate-btn" href="%1$s"><span class="asenha-2fa-btn-text">%2$s</span><span class="spinner asenha-2fa-btn-spinner" style="display:none;"></span><span class="asenha-2fa-btn-checkmark dashicons dashicons-yes" style="display:none;"></span></a>',
					esc_url( $url ),
					esc_html__( 'Revalidate now', 'admin-site-enhancements' )
				)
			);
		}

		if ( empty( $providers ) ) {
			$notices['notice two-factor-notice-no-providers-supported'] = esc_html__( 'No providers are available for your account.', 'admin-site-enhancements' );
		}
		...
		if ( count( $providers ) > 1 && 1 === count( $enabled_providers ) ) {
			$notices['warning two-factor-warning-suggest-backup'] = esc_html__( 'To prevent being locked out of your account, consider enabling a backup method like Recovery Codes in case you lose access to your primary authentication method.', 'admin-site-enhancements' );
		}
		?>
		<h2><?php esc_html_e( 'Two-Factor Authentication Options', 'admin-site-enhancements' ); ?></h2>
	...
	private static function render_user_providers_form( $user, $providers ) {
		...
		/*
		 * Ensure the methods appear in a predictable order for ASE:
		 * - Authenticator App (TOTP)
		 * - Recovery Codes
		 * - Email
		 *
		 * This keeps the "Recommended" labels controlled by `get_recommended_providers()`,
		 * while placing Email below Recovery Codes as requested.
		 */
		$asenha_provider_order = array(
			'Two_Factor_Totp'         => 0,
			'Two_Factor_Backup_Codes' => 1, // Labelled as "Recovery Codes" in Two-Factor.
			'Two_Factor_Email'        => 2,
		);

		uasort(
			$providers,
			static function( $provider_a, $provider_b ) use ( $asenha_provider_order ) {
				$class_a = is_object( $provider_a ) ? get_class( $provider_a ) : '';
				$class_b = is_object( $provider_b ) ? get_class( $provider_b ) : '';

				$order_a = isset( $asenha_provider_order[ $class_a ] ) ? $asenha_provider_order[ $class_a ] : 999;
				$order_b = isset( $asenha_provider_order[ $class_b ] ) ? $asenha_provider_order[ $class_b ] : 999;

				if ( $order_a === $order_b ) {
					return strcasecmp( (string) $class_a, (string) $class_b );
				}
		...
		?>
		<p>
			<?php esc_html_e( 'Configure a primary two-factor method along with a backup method, such as Recovery Codes, to avoid being locked out if you lose access to your primary method. Methods marked as recommended are more secure and easier to use.', 'admin-site-enhancements' ); ?>
		</p>
		...
		<p>
			<?php esc_html_e( 'Authentication for REST API and XML-RPC must use application passwords (defined above) instead of your regular password.', 'admin-site-enhancements' ); ?>
		</p>
					...
					<td>
						<label class="two-factor-method-label">
							...
							<strong><?php echo esc_html( sprintf( 
									/* translators: %s: label for the 2FA method */
									__( 'Enable %s', 'admin-site-enhancements' ), 
									$object->get_label() 
								) ); ?></strong>
							<?php if ( in_array( $provider_key, $recommended_provider_keys, true ) ) : ?>
								<abbr title="<?php esc_attr_e( 'This method is more secure and easy to use', 'admin-site-enhancements' ); ?>" class="two-factor-method-recommended"><?php esc_html_e( 'Recommended', 'admin-site-enhancements' ); ?></abbr>
							<?php endif; ?>
						</label>
						<?php
						/**
		...
		<table class="form-table two-factor-primary-method-table" role="presentation">
			<tbody>
				<tr>
					<th><?php esc_html_e( 'Primary Method', 'admin-site-enhancements' ); ?></th>
					<td>
						<select name="<?php echo esc_attr( self::PROVIDER_USER_META_KEY ); ?>">
							<option value=""><?php echo esc_html( __( 'Choose one', 'admin-site-enhancements' ) ); ?></option>
							<?php foreach ( $providers as $provider_key => $object ) : ?>
						...
						<p class="description"><?php esc_html_e( 'Select the primary method to use for two-factor authentication when signing into this site.', 'admin-site-enhancements' ); ?></p>
					</td>
				</tr>
			</tbody>
		</table>
	...
	/**
	 * Enqueue admin assets for Two-Factor UI actions.
	 *
	 * @since 8.2.4
	 *
	 * @return void
	...
	public static function enqueue_admin_assets() {
		wp_enqueue_style(
			'asenha-two-factor-buttons',
			plugins_url( 'assets/css/two-factor-buttons.css', __FILE__ ),
			array(),
			TWO_FACTOR_VERSION
		);
		wp_enqueue_script(
			'asenha-two-factor-buttons',
			plugins_url( 'assets/js/two-factor-buttons.js', __FILE__ ),
			array( 'jquery' ),
			TWO_FACTOR_VERSION,
			true
		);
	...
	/**
	 * Enqueue login assets for Two-Factor UI actions.
	 *
	 * @since 8.2.4
	 *
	 * @return void
	...
	public static function enqueue_login_assets() {
		...
		$spinner_url = includes_url( 'images/spinner-2x.gif' );
		$inline_css  = 'body.login .asenha-2fa-btn-spinner{background-image:url("' . esc_url( $spinner_url ) . '");background-repeat:no-repeat;background-position:center;}';
		wp_add_inline_style( 'asenha-two-factor-buttons', $inline_css );
	}
