 ...
 * @var null|EditableRolesFeature $editableRolesFeature
 */
...
<div id="ame-role-editor-root">
	...
	<div id="rex-main-ui" data-bind="visible: isLoaded, css: {'rex-highlight-changes': highlightUnsavedChangesEnabled}"
		...
		<div id="rex-content-container">
			...
			<div id="rex-category-sidebar">
				...
				<ul data-bind="template: {name: 'rex-nav-item-template', data: rootCategory},
					css: {
						'rex-fancy-capability-counts': colorizeCapCountEnabled,
						'rex-with-capability-count-bars': showCapCountProgressEnabled
					}"
		...
		<div id="rex-action-sidebar" class="metabox-holder">
			<div id="rex-main-buttons" class="postbox ws-ame-postbox">
				...
				<?php
				submit_button(
					'Editable roles' . ($isProVersion ? '' : ' (Pro)'),
					'rex-action-button',
		...
		<div id="rex-category-list-options" class="rex-dropdown" style="display: none;">
			...
			<div class="rex-dropdown-item rex-dropdown-sub-item-divider"></div>
			<label class="rex-dropdown-item rex-dropdown-sub-item">
				<input type="checkbox" data-bind="checked: colorizeCapCountEnabled, enable: showNumberOfCapsEnabled">
				Show as badges
			</label>
			<label class="rex-dropdown-item rex-dropdown-sub-item">
				<input type="checkbox" data-bind="
					checked: showCapCountProgressEnabled,
					enable: showNumberOfCapsEnabled() && colorizeCapCountEnabled()">
				Show progress bars
			</label>
		</div>
		...
		<div id="rex-editable-roles-dialog" title="Editable roles" class="rex-dialog"
			...
			<!-- ko if: (editableRolesDialog && editableRolesDialog.visibleActors) -->
			<!-- ko template: {
			...
			<!-- /ko -->
			<!-- /ko -->
			<!-- ko ifnot: (editableRolesDialog && editableRolesDialog.visibleActors) -->
			<!-- ko template: {
				name: 'rex-editable-roles-placeholder-template',
				data: editableRolesDialog
			} -->
			<!-- /ko -->
			<!-- /ko -->
			...
			<div class="ws_dialog_subpanel rex-dialog-section">
				...
				<fieldset>
					<p>
						<label>
							<input type="checkbox"
							       data-bind="checked: userScreensDialog.settings.otherRolesOnEditScreen">
			...
			<div class="ws_dialog_subpanel rex-dialog-section">
				...
				<fieldset>
					<p>
						<label>
							<input type="checkbox"
							       data-bind="checked: userScreensDialog.settings.capabilitiesLinkInUsersTable">
	...
	<script type="text/html" id="rex-nav-item-template">
		...
		<span class="rex-nav-toggle" data-bind="
			...
			<span class="rex-capability-count"
			      ...
			      style: {
			        '--ame-rex-enabled-cap-percent': (enabledCapabilityPercentage() + '%')
			      },
	...
	<?php
	if ( $editableRolesFeature ) {
		$editableRolesFeature->outputTemplate();
	}
	?>

	<script type="text/html" id="rex-editable-roles-placeholder-template">
		<div id="rex-editable-roles-placeholder-content">
			<h4>This is a Pro version feature. Here's what it does:</h4>
			<p>
				In WordPress, when someone has permission to edit users, they can usually
				edit all users and assign any role to any user. The "Editable roles" feature
				lets you restrict that on a per-role basis.
			</p>

			<ul>
				<li>
					Disabled roles won't appear in role dropdowns on the "Edit User"
					and "Add User" screens.
				</li>
				<li>
					Disabled roles will be removed from the role list on the "Users → All Users" page.
				</li>
				<li>
					When a user isn't allowed to edit/assign a particular role, they also
					won't be able to edit users who have that role.
				</li>
			</ul>
			<p id="rex-editable-roles-pro-cta">
				<a href="https://adminmenueditor.com/" target="_blank">
					Upgrade to Admin Menu Editor Pro
