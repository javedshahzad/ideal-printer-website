...
class DslFunctions {
	...
	public static function runCompare($args) {
		...
		switch ($operator) {
			...
			case '<=':
				...
				break;
			case 'in':
				$result = in_array($value1, $value2); //Intentionally non-strict comparison to allow for type juggling.
				break;
			default:
	...
	public static function runNot($args): bool {
		$value = $args['value'];
		return !$value;
	}

	public static function runDiv($args): float {
		$value1 = $args['numerator'];
		$value2 = $args['denominator'];

		if ( StyleGenerator::isEmptyCssValue($value1) || StyleGenerator::isEmptyCssValue($value2) ) {
			return 0.0;
		}

		if ( floatval($value2) == 0.0 ) {
			return NAN;
		}
	...
	public static function runCeil($args): ?float {
		$value = $args['value'];
